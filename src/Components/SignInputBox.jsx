    ...
    return (
      <div>
        <input 
          ...
          onChange={handleChange}
          placeholder={placeholder}
          className={`border-b-2 outline-none h-14 contain ${width} ${overallValidity ? 'placeholder:text-green-700 border-green-600 bg-transparent' : 'placeholder:text-red-500 border-red-400 bg-red-300 bg-opacity-20'}`}
        />
        {showError && (
          <p className="text-red-300 text-xs">
            {confirmError ? differentErrorMessage : errorMessage}
          </p>
        )}
      </div>
    );
